...
- **Script generation engine** — Anthropic Claude Sonnet 4.5 with `messages.parse` + `zodOutputFormat`. Voice DNA system: amplifies creator's tone 20% beyond their natural voice. Master system prompt (cached 1hr) with 7 psychological hook triggers (curiosity gap, pattern interrupt, self-identification, novelty detection, FOMO, social currency, arousal response), 12 non-negotiable writing rules (6th grade reading level, spoken language, fragments, self-corrections, specific details), 19 banned AI phrases, shareability engine (debate moments, specific sharing CTAs, loop mechanics for replay), and cross-prompt consistency rules. Deep intelligence per niche (10), platform (3), format (7), length (3), and pace (3).
...
| 2026-04-11 | AI system v3 — Voice DNA, psychology, shareability |
| 2026-04-11 | AI system v4 (10/10 rewrite) — condensed system prompt with 2 real viral examples, streaming via messages.stream(), Haiku for captions+scenes (3x faster), Sonnet for scripts (quality), format-specific example scripts for all 7 formats, cross-builder metadata (caption knows hook/CTA, scene regen knows length/pace/format/topic, remix gets target format selection), series episode compression, caption knows emotional arc |
| 2026-04-11 | Streaming UI: SSE endpoint at /api/generate streams text deltas to frontend in real-time during Own Idea generation. useScriptStream hook handles SSE parsing. Live preview panel shows script building as it writes. Remix gets targetFormat param for intelligent restructuring. Scene regen gets topicDescription to stay on-topic. |
