...
| 2026-04-11 | AI system v4 (10/10 rewrite) — condensed system prompt with 2 real viral examples, streaming via messages.stream(), Haiku for captions+scenes (3x faster), Sonnet for scripts (quality), format-specific example scripts for all 7 formats, cross-builder metadata (caption knows hook/CTA, scene regen knows length/pace/format, remix knows source format), series episode compression (full scenes for last 2, summaries for older), caption knows emotional arc via hook+CTA extraction |
